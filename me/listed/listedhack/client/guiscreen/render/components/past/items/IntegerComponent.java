...
public class IntegerComponent extends Component {
   ...
   public void renderComponent() {
      ...
      Gui.func_73734_a(this.parent.parent.getX() + 1, this.parent.parent.getY() + this.offset, this.parent.parent.getX() + this.parent.parent.getWidth() - 1, this.parent.parent.getY() + this.offset + 15, -13684945);
   ...
   public void updateComponent(int mouseX, int mouseY) {
      ...
      this.sliderWidth = (double)(100 * (this.set.get_value(1) - min) / (max - min));
      if (this.dragging) {
         if (diff == 0.0D) {
            this.set.set_value(this.set.get_min(1));
         } else {
            int newValue = (int)roundToPlace(diff / 100.0D * (double)(max - min) + (double)min, 2);
