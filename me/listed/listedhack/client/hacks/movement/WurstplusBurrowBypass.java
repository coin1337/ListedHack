...
public class WurstplusBurrowBypass extends WurstplusHack {
   private BlockPos originalPos;
   private int oldSlot = -1;
   private int itemSwitch = 0;
   WurstplusSetting mode = this.create("Mode", "Mode", "Chest", this.combobox(new String[]{"Chest", "Sand", "Echest"}));
   ...
   public WurstplusBurrowBypass() {
      super(WurstplusCategory.WURSTPLUS_MOVEMENT);
      this.name = "Burrow Bypass";
      this.tag = "BurrowBypass";
      this.description = "burrow with sand";
   }
   ...
   public void update() {
      if (this.mode.in("Chest") && InventoryUtil.findHotbarBlock(BlockChest.class) == -1) {
         this.toggle();
      }

      if (this.mode.in("Sand") && InventoryUtil.findHotbarBlock(BlockSand.class) == -1) {
         this.toggle();
      }

      if (this.mode.in("Echest") && InventoryUtil.findHotbarBlock(BlockEnderChest.class) == -1) {
         this.toggle();
      }
      ...
      if (this.mode.in("Chest")) {
         InventoryUtil.switchToSlot(InventoryUtil.findHotbarBlock(BlockChest.class));
      }

      if (this.mode.in("Sand")) {
         InventoryUtil.switchToSlot(InventoryUtil.findHotbarBlock(BlockSand.class));
      }

      if (this.mode.in("Echest")) {
         InventoryUtil.switchToSlot(InventoryUtil.findHotbarBlock(BlockEnderChest.class));
      }
